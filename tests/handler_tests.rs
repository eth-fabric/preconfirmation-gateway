...
use preconfirmation_gateway::types::{CommitmentRequest, RpcContext, DatabaseContext};
...
async fn test_commitment_request_handler_with_local_signer() -> eyre::Result<()> {
	...
	let proxy_address = commit_config.signer_client.generate_proxy_key_ecdsa(test_bls_pubkey).await
	...
	let inclusion_payload = InclusionPayload {
		slot: 12345,
		signed_tx: vec![0x01, 0x02, 0x03, 0x04].into(),
	};
	...
	let test_request = CommitmentRequest {
		commitment_type: 1,
		payload: encoded_payload,
		slasher: Address::from([0x2; 20]),
	};
	...
	// Call the handler
	let result = commitment_request_handler::<()>(
		params,
		context,
		Extensions::default(),
	).await;
