...
pub mod ISlasher {
	...
	impl alloy_sol_types::SolInterface for ISlasherCalls {
		...
		fn abi_decode_raw(selector: [u8; 4], data: &[u8]) -> alloy_sol_types::Result<Self> {
			static DECODE_SHIMS: &[fn(&[u8]) -> alloy_sol_types::Result<ISlasherCalls>] = &[{
				fn slash(data: &[u8]) -> alloy_sol_types::Result<ISlasherCalls> {
					<slashCall as alloy_sol_types::SolCall>::abi_decode_raw(data).map(ISlasherCalls::slash)
				}
				slash
			}];
		...
		fn abi_decode_raw_validate(selector: [u8; 4], data: &[u8]) -> alloy_sol_types::Result<Self> {
			static DECODE_VALIDATE_SHIMS: &[fn(&[u8]) -> alloy_sol_types::Result<ISlasherCalls>] = &[{
				fn slash(data: &[u8]) -> alloy_sol_types::Result<ISlasherCalls> {
					<slashCall as alloy_sol_types::SolCall>::abi_decode_raw_validate(data).map(ISlasherCalls::slash)
				}
				slash
			}];
			let Ok(idx) = Self::SELECTORS.binary_search(&selector) else {
	...
	impl<P: alloy_contract::private::Provider<N>, N: alloy_contract::private::Network> ISlasherInstance<P, N> {
		...
		pub fn slash(
			&self,
			delegation: <Delegation as alloy::sol_types::SolType>::RustType,
			commitment: <Commitment as alloy::sol_types::SolType>::RustType,
